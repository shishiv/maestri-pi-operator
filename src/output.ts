...
function normalizeUntrustedText(input: string): string {
...
export function redactSensitiveText(input: string, env: Environment): string {
	...
	const values = Object.entries(env)
		.filter(([name, value]) => Boolean(value) && isSensitiveEnvironmentName(name))
		.map(([, value]) => normalizeUntrustedText(value as string))
		.filter(Boolean)
